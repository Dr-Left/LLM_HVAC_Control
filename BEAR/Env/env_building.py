...
class BuildingEnvReal(gym.Env):
    ...
    Attributes:
        ...
        room_names: list of room names from the building configuration
    """
    ...
    def __init__(
        ...
        Args:
            Parameter (dict): Dictionary containing the parameters for the environment.
                ...
                'buildall' (list): List of room information including names.
        Initializes:
        ...
        if noise > 0:
            self.OutTemp = self.OutTemp + np.random.normal(0, noise, self.OutTemp.shape)
        ...
        # Extract room names from buildall
        self.room_names = [room[0] for room in Parameter["buildall"]]

        # Define action space bounds based on room number and air conditioning map
        self.Qlow = (
            np.ones(self.roomnum, dtype=np.float32)
            # * (-1.0)
            * 0
            * self.acmap.astype(np.float32)
        )
        self.Qhigh = (
            np.ones(self.roomnum, dtype=np.float32)
            * (1.0)
        ...
        if reward_breakdown_keys is not None and isinstance(
            reward_breakdown_keys, list
        ):
    ...
    def step(
        ...
        return (
            ...
            info,
        )  # double done is for gymnasium grammar

    def reset(
        self, *, epochs=0, seed: int | None = None, options: dict | None = None
